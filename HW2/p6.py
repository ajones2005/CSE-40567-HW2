...
def f_function(r, k):
    ...
    f_out = permute(s_out, p)
    return f_out

def des_decrypt_block(cipher64_bits, key64_bits):
    enc_round_keys = generate_round_keys(key64_bits)
    round_keys = list(reversed(enc_round_keys))
    
    ip_out = permute(cipher64_bits, ip)
    l = ip_out[:32]
    r = ip_out[32:]
    
    print("l0:", ''.join(map(str,l)))
    print("r0:", ''.join(map(str,r)))
    ...
    for i in range(16):
        ...
        print(f"round {i+1} f: {''.join(map(str,f_out))}\nl{i+1}: {''.join(map(str,new_l))}\nr{i+1}: {''.join(map(str,new_r))}\n")
...
def bitstring_to_bits(s):
    return [1 if c == '1' else 0 for c in s]

if __name__ == "__main__":
    cipher_bits = bitstring_to_bits(
        "1100101011101101101000100110010101011111101101110011100001110011"
    )
    key_bits = bitstring_to_bits(
        "0100110001001111010101100100010101000011010100110100111001000100"
    )
    
    plaintext_bits = des_decrypt_block(cipher_bits, key_bits)
    print("plaintext bits:", ''.join(map(str, plaintext_bits)))
    print("text:", bitlist_to_str(plaintext_bits).lower())
